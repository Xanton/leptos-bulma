...
const NOTIFICATION_LIGHT_COLOR_OPTIONS: [BColor; 7] = [
    BColor::Danger,
    BColor::Default,
    BColor::Info,
    BColor::Link,
...
#[component]
pub fn NotificationLightColors() -> impl IntoView {
    view! {
        <For
            each=move || NOTIFICATION_LIGHT_COLOR_OPTIONS
            key=|bcolor| bcolor.clone()
            children=move |bcolor| {
                let is_active = RwSignal::new(true);
                let color = StoredValue::new(bcolor.clone());
                view! {
                    <BNotification color=bcolor.clone() is_light=true is_active=is_active>
